...
def addWeight(request):
    today = date.today()
    existing_entry = WeightEntry.objects.filter(user=request.user, date=today).first()
    if request.method == 'POST':
...
def addedWeight(request):
    weight_list = WeightEntry.objects.filter(user=request.user)
    return render(request, 'retrieve.html', {'weight_list': weight_list})

@login_required
def weight_update(request, pk):
    weight = get_object_or_404(WeightEntry, pk=pk)
...
def weight_list(request):
    ...
    paginator = Paginator(weight_entries, 3)  
...
def weight_loss_calculator(request):
    if request.method == 'POST':
        ...
        try:
            start_weight = WeightEntry.objects.get(user=request.user, date=start_date)
            end_weight = WeightEntry.objects.get(user=request.user, date=end_date)
